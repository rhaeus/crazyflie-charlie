...
def matrixToPose(T):
    ...
    o_T_l = poseToMatrix(marker_in_odom.pose.position,marker_in_odom.pose.orientation)
    l_T_o = tf.transformations.inverse_matrix(o_T_l)
    m_T_l = mMapMatrix(m)

    if m_T_l is None:
        print("ID does not exist in world")
        return
