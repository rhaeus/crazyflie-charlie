...
class Kalman2:
    #https://github.com/xaedes/ROS-Kalman-Filter-for-IMU/blob/master/scripts/kalman.py
    def __init__(self,xcord=[1,2,0,0,0,0,1],P =np.identity(6),Q=np.identity(6),R=np.identity(6)):
        self.x = [None]*6           # x = prior estimate, mean 
        self.x[0:3] = xcord[0:3]
        self.x[3:6] = tf.transformations.euler_from_quaternion(xcord[3:7])
        self.x = np.array(self.x)
        self.P = P                  # P = covariance of estimate
        self.Q = Q                  # Q = covariance of process noise
        self.R = R*0.1              # R = covariance of measurment noise
        self.A = np.identity(6)     # A = state transition model, identity 
        self.C = np.ones(6)         # C = Observation model
    ...
    def predict(self):
        std = 0.001
        for i in range(3):
            self.Q[i,i] = np.random.normal(0,std)
        std = 5
        for i in range(3,6):
            self.Q[i,i] = np.random.normal(0,std)
        self.Q=np.identity(6)*0.01
        #self.Q[0,0] = 0.01
        #self.Q[1,1] = 0.01
        #self.Q[5,5] = 0.01 
    ...
    def update(self,msg):
        xt = msg.transform.translation.x
        yt = msg.transform.translation.y
        zt = msg.transform.translation.z
        angles = tf.transformations.euler_from_quaternion([msg.transform.rotation.x,msg.transform.rotation.y,msg.transform.rotation.z,msg.transform.rotation.w])
        Z = np.array([xt,yt,zt,angles[0],angles[1],angles[2]]) # Z = measurement
        ...
        x,y,z,w = tf.transformations.quaternion_from_euler(0,0,self.x[5])
