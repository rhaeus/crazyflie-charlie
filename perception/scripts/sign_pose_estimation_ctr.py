        ...
        #get contours
            ...
            if image_points != [[]]:
                ...
                for i in self.world['roadsigns']:
                    if i['sign']==sign_name:
                        map_position = i['pose']['position']
                        map_orient = i['pose']['orientation']

                initial_guess.pose.position.x = map_position[0]
                initial_guess.pose.position.y = map_position[1]
                initial_guess.pose.position.z = map_position[2]
                (initial_guess.pose.orientation.x,
                initial_guess.pose.orientation.y,
                initial_guess.pose.orientation.z,
                initial_guess.pose.orientation.w) = quaternion_from_euler(math.radians(map_orient[0]-90),
                                                                math.radians(map_orient[1]),
                                                                math.radians(map_orient[2]-90))
                ...
                #    object_points[:4], image_points[:4], self.camera_matrix, self.distortion, guess_r, guess_t, True, cv.SOLVEPNP_ITERATIVE)
                (_, rotation_vector, translation_vector) = cv.solvePnP(
