...
def pose_callback(msg):
    ...
    if not tf_buf.can_transform(msg.header.frame_id, 'map', msg.header.stamp):
...
def pose_stamped_from_marker(m):
    ...
    p.pose.position.z = p.pose.position.z + 0.4
...
def main(argv=sys.argv):
    ...
    global set_points
    set_points = [pose_stamped_from_marker(m) for m in world['markers']]

    global set_point_count
    set_point_count = len(set_points)


    # rospy.loginfo(set_point_count)
    rospy.spin()
